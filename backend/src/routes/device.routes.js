...
router.post('/',
  ...
  [
    ...
    body('daily_limit').optional().isInt({ min: 1, max: 1000 }).withMessage('Daily limit must be between 1 and 1000'),
  ],
  validate,
  async (req, res) => {
    try {
      const { device_label, phone_number, daily_limit } = req.body;
      ...
      const deviceDailyLimit = daily_limit || 50; // Default to 50 if not provided
...
// ============================================================================
// NEW: ENHANCED DEVICE MANAGEMENT ENDPOINTS
// ============================================================================
...
router.get('/:id/health',
  ...
  async (req, res) => {
    try {
      ...
      const healthData = await DeviceHealthMonitor.calculateHealthScore(id);
...
// Get health summary for all user devices
router.get('/health/summary',
  verifyToken,
  async (req, res) => {
    try {
      const healthSummary = await DeviceHealthMonitor.getHealthSummary(req.user.id);
...
router.post('/:id/auto-heal',
  ...
  async (req, res) => {
    try {
      ...
      const healResult = await DeviceHealthMonitor.autoHealDevice(id);

      res.json({
        success: true,
        heal_result: healResult,
        message: 'Auto-heal process completed',
      });

    } catch (error) {
      console.error('Error auto-healing device:', error);
      res.status(500).json({ error: 'Failed to auto-heal device' });
    }
  }
);

// Update device daily limit
router.put('/:id/daily-limit',
  verifyToken,
  [
    param('id').isInt().withMessage('Device ID must be an integer'),
  ...
  async (req, res) => {
    try {
      ...
      await device.update({ daily_limit });
...
// Reset device daily counter (admin function)
router.post('/:id/reset-daily-counter',
  verifyToken,
  [
    param('id').isInt().withMessage('Device ID must be an integer'),
  ],
  ...
  async (req, res) => {
    try {
      ...
      await device.update({ messages_sent_today: 0 });
      ...
      res.json({
        ...
        device: {
          ...
          daily_limit: device.daily_limit,
          remaining_capacity: device.daily_limit,
        },
...
// Get device optimal timing
router.get('/:id/optimal-timing',
  verifyToken,
  [
    param('id').isInt().withMessage('Device ID must be an integer'),
    query('message_type').optional().isIn(['business', 'personal', 'international']),
  ],
  validate,
  async (req, res) => {
    try {
      const { id } = req.params;
      const messageType = req.query.message_type || 'business';
      ...
      const optimalTiming = await SmartScheduler.calculateOptimalTiming(id, messageType);
